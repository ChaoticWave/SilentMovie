<?php namespace ChaoticWave\SilentMovie\Cache;
...
class ElasticsearchStore extends TaggableStore implements Store
{
    ...
    {
        $_document = new CachedDataDocument($this->index, ['cache_key' => $key, 'cache_value' => $value], $this->getType(), $minutes);
        $_document->setId($this->prefix . $key);

        $_params = $_document->toParamsArray([], true);

        $this->client->index($_params);
    }
     ...
     * @return array
     */
    protected function buildParams(array $data, $all = false)
    {
        return array_merge([
            'index' => $this->index,
            'type'  => $this->getType($all),
        ],
            $data);
    }
     ...
     * @return string
     */
    public function getType($all = false)
    {
        return $this->type ?: ($all ? '_all' : static::DEFAULT_TYPE);
    }
}
